...
const SCRIPT_URL =
  "https://script.google.com/macros/s/AKfycbwFqELswduE-2VCWMPnYp5Z2YpOObxhyQrGnn-OWlryabmWhf4EzTCeYZBelhLllK5YkQ/exec";

// --- ข้อมูลจำลอง (Mock Data) สำหรับพรีวิวในกรณีที่ยังไม่ได้ใส่ SCRIPT_URL ---
const initialTasks = [
  {
    id: "at-1",
    owner: "อธิวัฒน์",
    collection: "คอลเล็กชัน CMU e-Newspaper",
    subTask: "แปลงข้อมูล (Digitization) ด้วยโปรแกรม Photoscape X",
    kpi: 3000,
    unit: "ไฟล์",
    months: {
      m6: 50,
  ...
  },
  {
    id: "at-2",
    owner: "อธิวัฒน์",
    collection: "คอลเล็กชันวิทยานิพนธ์",
    subTask: "สืบค้นและตรวจสอบระเบียนวิทยานิพนธ์",
    kpi: 1000,
    unit: "ระเบียน",
    months: {
      m6: 200,
  ...
  },
  {
    id: "py-1",
    owner: "ภิญญาพัชญ์",
    collection: "การพัฒนาทรัพยากรสารสนเทศ",
    subTask: "จัดซื้อฐานข้อมูลอิเล็กทรอนิกส์",
    kpi: 13,
    unit: "ฐาน",
    months: {
      m6: 2,
...
const monthNames = [
  ...
  { id: "m9", name: "กันยายน" },
  { id: "m10", name: "ตุลาคม" },
  { id: "m11", name: "พฤศจิกายน" },
  { id: "m12", name: "ธันวาคม" },
  { id: "m1", name: "มกราคม" },
  { id: "m2", name: "กุมภาพันธ์" },
  { id: "m3", name: "มีนาคม" },
  { id: "m4", name: "เมษายน" },
  { id: "m5", name: "พฤษภาคม" },
];
...
const userColorPalette = [
  ...
  },
  {
    ...
    solidStroke: "#14b8a6",
    bg: "bg-gradient-to-r from-teal-400 to-emerald-600",
    text: "text-teal-600",
    lightBg: "bg-teal-50/70",
    hover: "hover:stroke-teal-400",
  ...
  },
  {
    ...
    solidStroke: "#06b6d4",
    bg: "bg-gradient-to-r from-cyan-400 to-sky-600",
    text: "text-cyan-600",
    lightBg: "bg-cyan-50/70",
    hover: "hover:stroke-cyan-400",
    badge: "bg-cyan-100 text-cyan-700",
  },
  {
    id: "fuchsia",
    stroke: "url(#grad-fuchsia)",
    solidStroke: "#d946ef",
    bg: "bg-gradient-to-r from-fuchsia-500 to-pink-600",
    text: "text-fuchsia-600",
    lightBg: "bg-fuchsia-50/70",
    hover: "hover:stroke-fuchsia-400",
  ...
  return (
    <div className="w-full mt-auto pt-4">
      ...
      <div className="w-full bg-slate-100 rounded-full h-3.5 p-[2px] border border-slate-200/50">
        <div
          className={`h-full rounded-full bg-gradient-to-r ${colorGradient} transition-all duration-500 ease-out shadow-sm`}
...
export default function App() {
  ...
  // ฟังก์ชันโหลดข้อมูล (ดึงจาก Sheet ถ้ามี URL, ใช้ Mock ถ้าไม่มี)
  useEffect(() => {
    const fetchDB = async () => {
      if (!SCRIPT_URL || SCRIPT_URL.trim() === "") {
        // ใช้ข้อมูล Mock Data เพื่อให้ทดสอบ UI ได้
        ...
        setSelectedUserDashboard(initialTasks[0].owner);
        setEntryForm((prev) => ({ ...prev, user: initialTasks[0].owner }));
        setIsLoading(false);
        return;
      }
      ...
      try {
        ...
        setApiError(
          "ไม่สามารถดึงข้อมูลจาก Google Sheets ได้ โปรดตรวจสอบ SCRIPT_URL"
        );
      } finally {
        setIsLoading(false);
      }
  ...
  const syncApi = async (action, payload) => {
    if (!SCRIPT_URL || SCRIPT_URL.trim() === "") return;
    setIsSyncing(true);
    try {
  ...
  const calculateTotal = (monthsObj) =>
    Object.values(monthsObj || {}).reduce((a, b) => a + (Number(b) || 0), 0);

  const handleAddTask = (e) => {
    e.preventDefault();
    const collectionName = newTaskForm.isNewCollection
      ? newTaskForm.newCollection.trim()
      : newTaskForm.collection.trim();
    if (!collectionName || !newTaskForm.subTask.trim() || !newTaskForm.kpi)
      return;

    const newId = `task-${Date.now()}`;
    const newTask = {
      id: newId,
      owner: selectedUserDashboard,
      collection: collectionName,
      subTask: newTaskForm.subTask.trim(),
      kpi: Number(newTaskForm.kpi),
      unit: newTaskForm.unit.trim() || "ครั้ง",
      months: {
        m6: 0,
        m7: 0,
        m8: 0,
  ...
  const handleAddUser = (e) => {
    ...
    if (
      ...
      !newUserForm.startingCollection.trim() ||
      !newUserForm.startingSubTask.trim()
    )
      return;
    ...
    const firstTask = {
      id: newId,
      owner: newUserForm.name.trim(),
      collection: newUserForm.startingCollection.trim(),
      subTask: newUserForm.startingSubTask.trim(),
      kpi: Number(newUserForm.startingKPI),
      unit: newUserForm.startingUnit.trim() || "ครั้ง",
      months: {
        m6: 0,
        m7: 0,
        m8: 0,
  ...
  const executeDeletion = () => {
    ...
    if (deleteConfirm.type === "task") {
      ...
      const remainingTasks = tasks.filter(
        (t) => t.owner !== deleteConfirm.targetId
      );
  ...
  const handleAddCategory = (e) => {
    ...
    syncApi("ADD_CATEGORY", newCat);
    setNewCategory({ name: "", color: "#3b82f6" });
  };

  const handleDeleteCategory = (catId) => {
    if (categories.length <= 1)
      return alert("ต้องมีหมวดหมู่อย่างน้อย 1 รายการ");
    const confirmDelete = window.confirm(
      "การลบหมวดหมู่นี้จะทำให้กิจกรรมทั้งหมดในหมวดหมู่นี้ถูกย้ายไปอยู่หมวดหมู่อื่น ยืนยันที่จะลบหรือไม่?"
    );
    ...
    setEvents(
      events.map((evt) =>
        evt.categoryId === catId
          ? { ...evt, categoryId: fallbackCategory.id }
          : evt
      )
    );

    syncApi("DELETE_CATEGORY", { id: catId, fallbackId: fallbackCategory.id });
  ...
  const renderOverview = () => {
      ...
      return (
        <div className="flex flex-col items-center justify-center py-20 bg-white rounded-3xl border border-slate-100 shadow-sm animate-in fade-in">
          ...
          <p className="text-slate-500 font-medium text-center max-w-md">
            ยังไม่มีข้อมูลในฐานข้อมูลของคุณ
            กดปุ่มเพิ่มผู้ปฏิบัติงานเพื่อเริ่มต้นการทำงานได้เลยครับ
          </p>
          <button
            onClick={() => setActiveTab("individual")}
            className="mt-8 bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-3 px-6 rounded-xl shadow-lg transition-all"
          >
            ไปเพิ่มผู้ปฏิบัติงาน
          </button>
        </div>
      );
    }
    ...
    const totalAchievedAll = tasks.reduce(
      (acc, curr) => acc + calculateTotal(curr.months),
      0
    );
    const overallAvgProgress =
      totalKPIAll > 0 ? ((totalAchievedAll / totalKPIAll) * 100).toFixed(1) : 0;
    ...
    const userStats = users.map((user, idx) => {
      ...
      const achieved = userTasks.reduce(
        (acc, curr) => acc + calculateTotal(curr.months),
        0
      );
      return {
        ...
        kpi,
        achieved,
        progress: kpi > 0 ? Number(((achieved / kpi) * 100).toFixed(1)) : 0,
        colorClass: userColorPalette[idx % userColorPalette.length],
      };
    });
        ...
        <div className="border-b border-slate-200/60 pb-4 flex justify-between items-end">
          <div>
            <h2 className="text-3xl font-black text-slate-800 tracking-tight">
              ภาพรวมการปฏิบัติงานของบุคลากร
            </h2>
            <p className="text-base text-slate-500 mt-1">
              วิเคราะห์ผลงานและสถิติแยกตามบุคคล
            </p>
          </div>
          {isSyncing && (
            <div className="text-xs font-bold text-indigo-500 flex items-center bg-indigo-50 px-3 py-1.5 rounded-full shadow-sm">
            ...
            <div className="flex justify-between items-start">
              <div>
                ...
                <p className="text-3xl font-black text-blue-600 mt-1">
                  {totalAchievedAll.toLocaleString()}
                </p>
              </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          <div className="lg:col-span-5 bg-white p-6 rounded-2xl border border-slate-100 shadow-sm flex flex-col justify-between items-center text-center">
            <div className="w-full text-left mb-4">
              <h3 className="font-extrabold text-slate-800 text-xl flex items-center">
                <span className="w-3 h-6 bg-blue-500 rounded-full mr-2.5"></span>
                สัดส่วนปริมาณผลงานสำเร็จ
              </h3>
            </div>
              ...
              {totalAchievedSum > 0 ? (
                <>
                  ...
                  >
                    ...
                    {userStats.map((stat) => {
                      const proportion =
                        (stat.achieved / totalAchievedSum) * 100;
                      ...
                      return (
                        <circle
                          ...
                          r={radius}
                          fill="transparent"
                          stroke={stat.colorClass.solidStroke}
                          strokeWidth="10"
                  ...
                  <div className="absolute inset-0 flex flex-col items-center justify-center bg-white rounded-full m-12 shadow-sm border border-slate-50">
                    ...
                    <span className="text-2xl font-black text-slate-800">
                      {totalAchievedSum.toLocaleString()}
                    </span>
                  </div>
                </>
              ) : (
            ...
            <div className="w-full grid grid-cols-2 gap-3 border-t border-slate-100 pt-4 mt-2">
              {userStats.map((stat) => (
                <div
                  key={stat.user}
                  className={`flex items-center space-x-2.5 text-left p-2 rounded-xl ${stat.colorClass.lightBg}`}
                >
                  <div
                    className={`w-4 h-4 rounded-full ${stat.colorClass.bg}`}
                  ></div>
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="text-xs font-semibold text-slate-500">
                      {totalAchievedSum > 0
                        ? ((stat.achieved / totalAchievedSum) * 100).toFixed(1)
                        : 0}
                      % ({stat.achieved.toLocaleString()})
                    </p>
                  </div>
                </div>
              ))}
          ...
          <div className="lg:col-span-7 space-y-4">
            <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm">
              <h3 className="font-extrabold text-slate-800 text-xl mb-4 flex items-center">
                <span className="w-3 h-6 bg-emerald-500 rounded-full mr-2.5"></span>
                ความสำเร็จเทียบ KPI รายบุคคล
              </h3>
              <div className="space-y-4">
                {userStats.map((stat) => (
                  <div
                    key={stat.user}
                    className="p-4 rounded-xl border border-slate-100/70 bg-slate-50/40"
                  >
                    <div className="flex justify-between items-center mb-3">
                      <div className="flex items-center space-x-2.5">
                        <span
                          className={`w-3.5 h-3.5 rounded-full ${stat.colorClass.bg}`}
                        ></span>
                      ...
                      <div className="text-right">
                        ...
                        <span
                          className={`text-base font-black ${stat.colorClass.text}`}
                        >
                          {stat.progress}%
                        </span>
                      </div>
  ...
  const renderIndividualDashboard = () => {
    ...
    const existingCollections = [
      ...new Set(userTasks.map((t) => t.collection)),
    ];
    const userIndex = users.indexOf(selectedUserDashboard);
    const activeColor =
      userColorPalette[
        userIndex !== -1 ? userIndex % userColorPalette.length : 0
      ];
      ...
      <div className="space-y-6 animate-in fade-in duration-300">
        <div className="flex flex-col lg:flex-row lg:items-center justify-between border-b border-slate-100 pb-4 gap-4">
            ...
            {selectedUserDashboard && (
              <span
                className={`px-3 py-1 rounded-full text-xs font-bold ${
                  activeColor?.badge || "bg-slate-100"
                } shadow-sm`}
              >
                {selectedUserDashboard}
              </span>
          ...
          <div className="flex flex-wrap items-center gap-3">
            {users.length > 0 ? (
              <select
                className="p-2.5 border border-slate-200 rounded-xl bg-white shadow-sm focus:ring-2 focus:ring-indigo-500 outline-none text-sm font-bold text-slate-700"
                value={selectedUserDashboard}
                onChange={(e) => setSelectedUserDashboard(e.target.value)}
              >
                {users.map((u) => (
              ...
              <span className="text-sm text-rose-500 bg-rose-50 px-3 py-1.5 rounded-lg font-bold">
                ไม่พบผู้ใช้งาน
              </span>
            )}
            ...
            <button
              ...
              className="bg-gradient-to-r from-emerald-500 to-teal-600 text-white px-4 py-2.5 rounded-xl hover:shadow-lg text-xs font-bold flex items-center"
            >
              <UserPlus size={14} className="mr-1.5" /> เพิ่มบุคลากร
            </button>
            <button
              onClick={() => {
                ...
                setNewTaskForm({
                  ...
                  isNewCollection: existingCollections.length === 0,
                });
              }}
              disabled={!selectedUserDashboard}
              className="bg-gradient-to-r from-blue-500 to-indigo-600 text-white px-4 py-2.5 rounded-xl disabled:from-slate-300 disabled:to-slate-300 text-xs font-bold flex items-center"
            >
              <Plus size={14} className="mr-1.5" /> เพิ่มภาระงาน
            </button>
            <button
              onClick={() =>
                setDeleteConfirm({
                  ...
                  displayName: `ผู้ปฏิบัติงาน "${selectedUserDashboard}"`,
                })
              }
              disabled={!selectedUserDashboard}
              className="bg-gradient-to-r from-rose-500 to-red-600 text-white px-4 py-2.5 rounded-xl disabled:from-slate-300 disabled:to-slate-300 text-xs font-bold flex items-center"
            >
              <Trash2 size={14} className="mr-1.5" /> ลบคนนี้
            </button>
          </div>
        </div>
        ...
        Object.keys(groupedByCollection).length > 0 ? (
            ...
            <div key={idx} className="mb-8">
              <h3
                className={`text-base font-black ${
                  activeColor?.text || "text-slate-800"
                } ${
                  activeColor?.lightBg || "bg-slate-100"
                } p-3.5 rounded-2xl border-l-4 border-l-current mb-4 shadow-sm flex items-center`}
              >
                <span className="w-1.5 h-4 bg-current rounded-full mr-2.5"></span>
                {collectionName}
              </h3>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 px-1 items-stretch">
                {groupedByCollection[collectionName].map((task) => {
                  ...
                  return (
                    <div
                      key={task.id}
                      className="bg-white border border-slate-100 rounded-2xl p-6 shadow-sm group flex flex-col h-auto hover:shadow-lg transition-all duration-300 relative overflow-hidden"
                    >
                        ...
                        <button
                          onClick={() =>
                            setDeleteConfirm({
                              type: "task",
                              targetId: task.id,
                              displayName: `ภาระงาน "${task.subTask}"`,
                            })
                          }
                          className="text-slate-300 hover:text-rose-600 p-1.5 opacity-0 group-hover:opacity-100 rounded-lg hover:bg-rose-50"
                        >
                          <Trash2 size={16} />
                        </button>
                      </div>
                      ...
                      <div className="flex-grow mb-6">
                        <h4 className="font-semibold text-slate-700 text-sm leading-relaxed whitespace-pre-wrap">
                          {task.subTask}
                        </h4>
                      </div>
                      ...
                      <div className="mt-auto">
                        <div className="flex justify-between items-center bg-slate-50/80 p-3.5 rounded-xl mb-4 border border-slate-100">
                          ...
                          <div className="text-center border-l border-slate-200/60 pl-3">
                            ...
                            <div className="font-black text-lg text-slate-600">
                              {task.kpi.toLocaleString()}{" "}
          ...
          <div className="bg-white border border-slate-200 rounded-2xl p-12 flex flex-col items-center justify-center text-slate-500 shadow-sm animate-in fade-in">
            ...
            <p className="text-sm">
              กดเพิ่มหัวข้อและภาระงานใหม่ที่ด้านบนขวาเพื่อเริ่มต้น
            </p>
          </div>
        )}
          ...
          <div className="fixed inset-0 bg-slate-900/40 flex items-center justify-center z-[110] p-4 backdrop-blur-sm">
            <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md flex flex-col overflow-hidden animate-in zoom-in-95">
              <div className="px-6 py-5 border-b border-slate-100 flex justify-between bg-emerald-50/80">
                <h3 className="font-black text-base text-emerald-800 flex items-center">
                  <UserPlus size={18} className="mr-2 text-emerald-500" />
                  เพิ่มผู้ปฏิบัติงานใหม่
                </h3>
                <button
                  onClick={() => setIsAddUserOpen(false)}
                  className="text-slate-400 hover:bg-slate-200 p-1 rounded-lg transition-colors"
                >
                  <X size={20} />
                </button>
              </div>
                ...
                <form onSubmit={handleAddUser} className="space-y-4">
                  <div>
                    ...
                    <input
                      type="text"
                      required
                      placeholder="เช่น ศิริพรรณ แสนสุข"
                      className="w-full p-3 border rounded-xl text-sm font-semibold focus:ring-2 focus:ring-emerald-500 outline-none"
                  ...
                  <div className="p-4 bg-slate-50 rounded-2xl border border-slate-100 space-y-3">
                    <span className="text-[10px] font-bold text-slate-400 uppercase border-b border-slate-200 pb-2 block mb-2">
                      ลงทะเบียนงานเริ่มต้น
                    </span>
                    <div>
                      <label className="text-[10px] font-bold text-slate-500 uppercase mb-1 block">
                        หัวข้อหลัก
                    ...
                    <div>
                      ...
                      <textarea
                        ...
                        className="w-full p-2.5 border rounded-xl text-xs font-semibold focus:ring-2 focus:ring-emerald-500 outline-none"
                        value={newUserForm.startingSubTask}
                        onChange={(e) =>
                          setNewUserForm({
                            ...newUserForm,
                            startingSubTask: e.target.value,
                          })
                        }
                      />
                    </div>
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        ...
                        <input
                          ...
                          min="1"
                          className="w-full p-2.5 border rounded-xl text-xs font-semibold focus:ring-2 focus:ring-emerald-500 outline-none"
                          value={newUserForm.startingKPI}
                          onChange={(e) =>
                            setNewUserForm({
                              ...newUserForm,
                              startingKPI: e.target.value,
                            })
                          }
                        />
                      </div>
                      <div>
                        ...
                        <input
                          ...
                          placeholder="ครั้ง"
                          className="w-full p-2.5 border rounded-xl text-xs font-semibold focus:ring-2 focus:ring-emerald-500 outline-none"
                          value={newUserForm.startingUnit}
                          onChange={(e) =>
                            setNewUserForm({
                              ...newUserForm,
                              startingUnit: e.target.value,
                            })
                          }
                        />
                      </div>
                  ...
                  <div className="flex gap-3 pt-2">
                    <button
                      type="button"
                      onClick={() => setIsAddUserOpen(false)}
                      className="flex-1 bg-slate-100 text-slate-700 py-3 rounded-xl text-xs font-bold hover:bg-slate-200 transition-all"
                    >
                      ยกเลิก
                    </button>
                    <button
                      type="submit"
                      className="flex-1 bg-gradient-to-r from-emerald-500 to-teal-600 text-white py-3 rounded-xl text-xs font-bold hover:from-emerald-600 hover:to-teal-700 transition-all shadow-md shadow-emerald-200"
                    >
                      บันทึกบุคลากรใหม่
                    </button>
                  </div>
          ...
          <div className="fixed inset-0 bg-slate-900/40 flex items-center justify-center z-[110] p-4 backdrop-blur-sm">
            <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md flex flex-col overflow-hidden animate-in zoom-in-95">
              <div className="px-6 py-5 border-b border-slate-100 flex justify-between bg-indigo-50/80">
                <h3 className="font-black text-base text-indigo-800 flex items-center">
                  <Plus size={18} className="mr-2 text-indigo-500" />
                  เพิ่มภาระงาน ({selectedUserDashboard})
                </h3>
                <button
                ...
                <form onSubmit={handleAddTask} className="space-y-4">
                  <div>
                    ...
                    <div className="flex gap-4 mb-3">
                      <label className="flex items-center text-xs font-bold cursor-pointer text-slate-700">
                        <input
                          ...
                          checked={!newTaskForm.isNewCollection}
                          onChange={() =>
                            setNewTaskForm({
                              ...newTaskForm,
                              isNewCollection: false,
                    ...
                    {!newTaskForm.isNewCollection ? (
                      <select
                        className="w-full p-3 border rounded-xl text-sm font-semibold focus:ring-2 focus:ring-indigo-500 outline-none bg-slate-50 hover:bg-white transition-colors"
                        value={newTaskForm.collection}
                        onChange={(e) =>
                          setNewTaskForm({
                            ...newTaskForm,
                      ...
                      <input
                        type="text"
                        required
                        placeholder="ชื่อหัวข้อหลักใหม่..."
                        className="w-full p-3 border rounded-xl text-sm font-semibold focus:ring-2 focus:ring-indigo-500 outline-none"
                        value={newTaskForm.newCollection}
                        onChange={(e) =>
                          setNewTaskForm({
                            ...newTaskForm,
                  ...
                  <div className="flex gap-3 pt-2">
                    ...
                    <button
                      type="submit"
                      className="flex-1 bg-gradient-to-r from-blue-500 to-indigo-600 text-white py-3 rounded-xl text-xs font-bold hover:from-blue-600 hover:to-indigo-700 transition-all shadow-md shadow-indigo-200"
                    >
                      บันทึกภาระงาน
                    </button>
                  </div>
      ...
      return (
        <div className="flex flex-col items-center justify-center py-20 bg-white rounded-3xl border border-slate-100 shadow-sm animate-in fade-in">
          ...
          <p className="text-slate-500 font-medium text-center">
            ต้องมีการสร้างภาระงานในหน้าแดชบอร์ดก่อน จึงจะสามารถบันทึกยอดสะสมได้
          </p>
        </div>
      );
    }
    ...
    return (
      <div className="max-w-2xl mx-auto space-y-6 animate-in fade-in duration-300">
        ...
        <div className="bg-gradient-to-r from-blue-50 to-indigo-50 text-indigo-900 p-5 rounded-2xl text-sm font-semibold border border-indigo-100 flex items-start shadow-sm">
          ...
          <span>
            ระบบจะนำผลงานใหม่ไป<strong>บวกสะสมเพิ่มเติม</strong>
            กับเดือนเดิมในฐานข้อมูล (Google Sheets) อัตโนมัติ
          </span>
        </div>

        <form
          onSubmit={handleSaveProgress}
          className="bg-white p-7 rounded-2xl shadow-sm border border-slate-100 space-y-5"
        >
          <div>
          ...
          <div className="grid grid-cols-2 gap-5">
            <div>
              ...
              <select
                ...
                onChange={(e) =>
                  setEntryForm((prev) => ({ ...prev, month: e.target.value }))
                }
              >
                {monthNames.map((m) => (
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setEntryForm((prev) => ({ ...prev, value: e.target.value }))
                }
                required
              />
            </div>
          ...
          <button
            type="submit"
            className="w-full bg-gradient-to-r from-blue-500 to-indigo-600 text-white font-black py-3.5 rounded-xl shadow-lg shadow-indigo-200 hover:scale-[1.02] transition-transform mt-2"
          >
            บันทึกยอดสะสม
          </button>
        </form>
            ...
            <div className="grid grid-cols-3 md:grid-cols-6 gap-3 text-center">
              {monthNames.map((m) => {
                const currentTask = tasks.find(
                  (t) => t.id === entryForm.taskId
                );
                const val = currentTask?.months?.[m.id] || 0;
                return (
                  <div
                    key={m.id}
                    className="bg-slate-50/70 p-3 rounded-xl border border-slate-100"
                  >
                    <div className="text-slate-400 font-bold text-[10px] uppercase mb-1">
                      {m.name}
                    </div>
  ...
  const renderCalendar = () => {
    ...
    const handleAddEvent = (e) => {
      ...
      if (newEvent.date && newEvent.title) {
        const finalCatId =
          newEvent.categoryId || categories[0]?.id || "general";
        const ev = {
          ...newEvent,
      ...
      <div className="space-y-6 relative animate-in fade-in duration-300">
        <div className="flex justify-between items-center border-b border-slate-200 pb-4">
          ...
          <div className="flex items-center space-x-2 bg-white p-1.5 rounded-xl shadow-sm border border-slate-200">
            <button
              ...
              className="p-2 text-indigo-500 rounded-lg hover:bg-indigo-50 transition-colors"
            >
              <ChevronLeft size={18} />
            </button>
            <select
              className="p-1.5 bg-transparent font-black text-sm outline-none text-slate-700 cursor-pointer"
              value={month}
              onChange={(e) =>
                setCalendarDate(new Date(year, Number(e.target.value), 1))
              }
            >
            ...
            <select
              className="p-1.5 bg-transparent font-black text-sm outline-none text-slate-700 cursor-pointer border-l pl-2 border-slate-200"
              value={year}
              onChange={(e) =>
                setCalendarDate(new Date(Number(e.target.value), month, 1))
              }
            >
        ...
        <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div className="flex flex-wrap gap-2 items-center">
            <span className="text-xs font-black text-slate-400 mr-2 uppercase">
              ตัวกรองป้ายกำกับ:
            </span>
            <button
            ...
            {categories.map((cat) => (
              <button
                ...
                className="px-3.5 py-1.5 rounded-full text-xs font-bold border flex items-center gap-2 transition-all shadow-sm"
                style={
              ...
              >
                <span
                  ...
                  style={{
                    backgroundColor:
                      selectedCategoryFilter === cat.id ? "#fff" : cat.color,
                  }}
                ></span>
                {cat.name}
              </button>
          ...
          <button
            ...
            className="text-xs font-bold text-slate-600 bg-slate-100 hover:bg-indigo-50 hover:text-indigo-600 px-4 py-2 rounded-xl flex items-center gap-2 transition-all"
          >
            <Tag size={14} /> จัดการป้ายกำกับ
          </button>
        </div>

        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2 bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
            <div className="grid grid-cols-7 gap-2.5 text-center font-black mb-4 text-slate-400 text-xs uppercase tracking-widest">
              ...
              <div>พฤ</div>
              <div>ศ</div>
              <div className="text-blue-400">ส</div>
            </div>
            <div className="grid grid-cols-7 gap-2.5">
              {blanks.map((b) => (
                <div
                  key={`blank-${b}`}
                  className="p-2 bg-slate-50/50 rounded-xl opacity-30 h-28"
                ></div>
              ))}
              {days.map((day) => {
                ...
                return (
                  <div
                    ...
                    className={`p-2.5 border rounded-xl h-28 flex flex-col items-start cursor-pointer transition-all ${
                      selectedDateFilter === dateStr
                        ? "bg-indigo-50 border-indigo-400 ring-2 ring-indigo-100 shadow-md"
                        : "border-slate-100 hover:border-indigo-300 hover:bg-slate-50/50"
                    }`}
                  >
                    <span
                      className={`text-xs font-black w-7 h-7 flex items-center justify-center rounded-lg mb-2 ${
                        dayEvents.length > 0
                    ...
                    <div className="w-full flex-1 overflow-y-auto space-y-1.5 scrollbar-none">
                      {dayEvents.map((evt) => {
                        ...
                        ) || { name: "ไม่มี", color: "#64748b" };
                        return (
                          <div
                            key={evt.id}
                            className="text-[10px] px-2 py-1 rounded-md truncate font-bold border shadow-sm"
                            style={{
              ...
              <form onSubmit={handleAddEvent} className="space-y-4">
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <input
                      type="date"
                      required
                      className="w-full p-2.5 border border-slate-200 rounded-xl focus:ring-2 focus:ring-indigo-500 outline-none text-xs font-semibold"
                      value={newEvent.date}
                      onChange={(e) =>
                ...
                <div>
                  ...
                  <input
                    type="text"
                    required
                    placeholder="เช่น ประชุมสาขา..."
                    className="w-full p-2.5 border border-slate-200 rounded-xl focus:ring-2 focus:ring-indigo-500 outline-none text-sm font-semibold"
                ...
                <button
                  type="submit"
                  className="w-full bg-slate-800 hover:bg-slate-900 text-white text-xs font-black py-3 rounded-xl transition-all shadow-md mt-2"
                >
                  เพิ่มลงปฏิทิน
                </button>
              </form>
          ...
          <div className="fixed inset-0 bg-slate-900/40 flex items-center justify-center z-50 p-4 backdrop-blur-sm">
            <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md flex flex-col max-h-[85vh] animate-in zoom-in-95 duration-200 overflow-hidden">
              <div className="px-6 py-5 border-b border-slate-100 flex justify-between bg-slate-50/80">
                <h3 className="font-black text-base text-slate-800 flex items-center">
                ...
                {events.filter(
                  ...
                  events
                    ...
                    .map((evt) => {
                      const cat = categories.find(
                        (c) => c.id === evt.categoryId
                      ) || { color: "#64748b", name: "ไม่มี" };
                      return (
                        <div
                          key={evt.id}
                          className="flex items-start justify-between p-4 border border-slate-100 rounded-2xl mb-4 bg-white shadow-sm hover:border-indigo-100 transition-colors"
                        >
                          <div>
                          ...
                          <button
                            onClick={() => {
                              ...
                              syncApi("DELETE_EVENT", { id: evt.id });
                            }}
            ...
            <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md flex flex-col overflow-hidden animate-in zoom-in-95">
              <div className="px-6 py-5 border-b border-slate-100 flex justify-between bg-slate-50/80">
                <h3 className="font-black text-base text-slate-800 flex items-center">
                  ...
                  จัดการป้ายกำกับหมวดหมู่
                </h3>
                <button
                  onClick={() => setIsManageCategoryOpen(false)}
                  className="text-slate-400 hover:bg-slate-200 p-1 rounded-lg"
                >
                  <X size={20} />
                </button>
              </div>
                ...
                >
                  <div className="flex-1">
                    ...
                    <input
                      type="text"
                      required
                      placeholder="เช่น อบรมภายนอก..."
                      className="w-full p-2.5 border rounded-xl text-xs font-semibold focus:ring-2 focus:ring-indigo-500 outline-none"
                  ...
                  <div className="w-16">
                    ...
                    <input
                      type="color"
                      className="w-full h-10 rounded-xl cursor-pointer p-0 border-0 bg-transparent"
                  ...
                  <button
                    type="submit"
                    className="bg-indigo-600 text-white px-5 h-10 rounded-xl text-xs font-bold hover:bg-indigo-700 shadow-md"
                  >
                    เพิ่ม
                  </button>
                </form>
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col md:flex-row font-sans relative">
      ...
      <aside className="w-full md:w-64 bg-gradient-to-b from-slate-950 to-indigo-950 text-white flex flex-col shadow-2xl z-10 flex-shrink-0">
        <div className="p-6 border-b border-slate-800/60">
          <h1 className="text-2.5xl font-black text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-purple-400 flex items-center">
            <PieChart className="mr-2 text-indigo-400" size={26} /> KPI
            SmartTrack
          </h1>
        </div>
        <nav className="flex-1 p-4 space-y-2">
          {["overview", "individual", "entry", "calendar"].map((tab) => (
            <button
              ...
              className={`w-full flex items-center px-4 py-3 rounded-xl text-xs font-black transition-all ${
                activeTab === tab
        ...
        <div className="fixed inset-0 bg-slate-900/40 flex items-center justify-center z-[100] p-4 backdrop-blur-sm">
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-sm p-8 text-center animate-in zoom-in-95">
            ...
            <p className="text-sm text-slate-500 mb-8 leading-relaxed">
              ข้อมูล <strong>{deleteConfirm.displayName}</strong>{" "}
              จะถูกลบออกจากสถิติรวม ไม่สามารถเรียกคืนได้
            </p>
            <div className="flex gap-3">
              <button
                onClick={() => setDeleteConfirm(null)}
                className="flex-1 bg-slate-100 hover:bg-slate-200 py-3 rounded-xl text-sm font-bold text-slate-700 transition-colors"
              >
                ยกเลิก
              </button>
              <button
                onClick={executeDeletion}
                className="flex-1 bg-rose-600 hover:bg-rose-700 text-white py-3 rounded-xl text-sm font-bold transition-colors shadow-md shadow-rose-200"
              >
                ยืนยันการลบ
              </button>
            </div>
